...
const AvaniFormsPage = () => {
  ...
  const toggleDetails = (id) => setExpandedRow((s) => (s === id ? null : id));
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pt-8 md:pt-4">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <button
          onClick={exportExcel}
          className="inline-flex items-center bg-gradient-to-r from-indigo-600 to-pink-500 text-white px-5 py-2.5 rounded-lg hover:from-indigo-700 hover:to-pink-600 transition-all shadow-md active:scale-95 text-sm md:text-base font-semibold"
        >
          <Download size={18} className="mr-2" />
          Export to Excel
        </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        <button onClick={() => navigate("/")} className="flex items-center justify-between p-4 rounded-2xl bg-white/90 border border-gray-100 shadow-sm hover:shadow-md transition">
          <div className="text-left">
            <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide">All Leads</h3>
            <p className="text-xs text-gray-400 mt-1">View & manage all incoming leads</p>
          </div>
          <Users size={24} className="text-indigo-500" />
        </button>

        <button onClick={() => navigate("/contacted-leads")} className="flex items-center justify-between p-4 rounded-2xl bg-white/90 border border-gray-100 shadow-sm hover:shadow-md transition">
          <div className="text-left">
            <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Contacted Leads</h3>
            <p className="text-xs text-gray-400 mt-1">Leads that have been followed up</p>
          </div>
          <PhoneCall size={24} className="text-teal-500" />
        </button>

        <button onClick={() => navigate("/analytics")} className="flex items-center justify-between p-4 rounded-2xl bg-white/90 border border-gray-100 shadow-sm hover:shadow-md transition">
          <div className="text-left">
            <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Analytics</h3>
            <p className="text-xs text-gray-400 mt-1">See stats & performance</p>
          </div>
          <BarChart3 size={24} className="text-pink-500" />
        </button>

        <button onClick={() => navigate("/avani-forms")} className="flex items-center justify-between p-4 rounded-2xl bg-gradient-to-br from-indigo-600 to-pink-500 text-white shadow-sm hover:shadow-md transition">
          <div className="text-left">
            <h3 className="text-xs font-semibold uppercase tracking-wide text-white">Submissions</h3>
            <p className="text-xs text-white/80 mt-1">Website contact forms</p>
          </div>
      ...
      <div className="bg-white/80 backdrop-blur-xl p-4 md:p-5 rounded-2xl border border-indigo-50 shadow-sm">
        <div className="relative w-full md:w-96">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-400/30 focus:border-indigo-400 bg-white/90 text-sm"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
          />
        </div>
        ...
        <div className="mt-3 text-xs text-gray-400">
          Showing <span className="font-semibold text-gray-600">{filtered.length}</span> submission{filtered.length !== 1 && "s"}.
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center text-red-600 bg-red-50 p-6 rounded-xl">{error}</div>
      ) : filtered.length === 0 ? (
        <div className="text-center text-gray-500 bg-white p-10 rounded-xl">No submissions found</div>
      ) : (
        <>
          {/* MOBILE VIEW */}
          <div className="md:hidden space-y-4">
            {filtered.map((f) => (
              <div key={f._id} className="bg-white p-4 rounded-2xl shadow-sm space-y-4">
                <div>
                  <h3 className="font-bold text-lg">{f.fullName || "N/A"}</h3>
                  <p className="text-xs text-gray-400">{f.createdAt ? new Date(f.createdAt).toLocaleString() : "—"}</p>
                </div>
                ...
                <button onClick={() => toggleDetails(f._id)} className="w-full flex justify-between items-center bg-indigo-50 p-3 rounded-xl text-sm font-semibold text-indigo-700">
                  Project Details
                  {expandedRow === f._id ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                </button>

                {expandedRow === f._id && <div className="bg-gray-50 p-3 rounded-xl text-sm">{f.projectDetails || "No details provided"}</div>}
              </div>
            ))}
          </div>
              ...
              <tbody>
                {filtered.map((f) => (
                  <tr key={f._id} className="border-t">
                    ...
                    <td className="px-6 py-4 text-xs text-gray-500">{f.createdAt ? new Date(f.createdAt).toLocaleString() : "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
